...
class MaxillaryViewScreen extends StatefulWidget {
  ...
  @override
  _MaxillaryViewScreenState createState() => _MaxillaryViewScreenState();
}
...
class _MaxillaryViewScreenState extends State<MaxillaryViewScreen> {
  ...
  Future<void> runObjectDetection(image, path) async {
    ...
    var pictureUrl = await Future.wait([
      uploadFileForUsers(File(image!.path), "$path-maxillaryView",
          "$path-maxillaryView-$uniqueNum", rects, true)
    ]);

    Future.wait([
      uploadFileForUsers(File(txtPath), "$path-maxillaryView",
          "$path-maxillaryView-$uniqueNum", rects, false)
    ]);

    // saveTxtToPicked(objDetect, txtPath);\
    setState(() {
      print("setState");
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Color.fromARGB(255, 209, 237, 255), // Background color
            ...
            child: Container(
              ...
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 5), // Space from the top
                  const Text(
