...
class MandibularViewScreen extends StatefulWidget {
  final String doctorName;
  final Map<String, dynamic> patientData;
  final String patientId;
  MandibularViewScreen(
      {Key? key,
  ...
  @override
  _MandibularViewScreenState createState() => _MandibularViewScreenState();
}
...
class _MandibularViewScreenState extends State<MandibularViewScreen> {
  ...
  Future<void> _pickImage() async {
    ...
    // });
    // final filePath = '/storage/emulated/0/Download/img2.png';
  ...
  Future<void> runObjectDetection(image, path) async {
    ...
    var pictureUrl = await Future.wait([
      uploadFileForUsers(File(image!.path), "$path-mandibularView",
          "$path-mandibularView-$uniqueNum", rects, true)
    ]);

    Future.wait([
      uploadFileForUsers(File(txtPath), "$path-mandibularView",
          "$path-mandibularView-$uniqueNum", rects, false)
    ]);
    // uploadFileForUsers(File(txtPath), "$patName-$age-$location",
    //     "$patName-$age-$location-$view-$uniqueNum");
                  ...
                  children: [
                    SingleChildScrollView(
                      ...
                      child: Row(
                        ...
                        children: [
                          const SizedBox(
                              width: 5), // Space at the start of the row
                          for (int i = 0; i < teeth[j].length; i++)
                            Row(
                              children: [
                                ...
                                if (i < teeth[j].length - 1) SizedBox(width: 3),
                              ],
                            ),
                        ],
                      ),
              ...
              Container(
                ...
                child: ElevatedButton(
                  onPressed: () {
                    ...
                    set_view_status("MandibularView");
